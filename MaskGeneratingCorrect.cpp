...
void closeContourn(std::vector<cv::Point> contours, std::vector<cv::Point> &interestPoints, bool &close) {
	...
	for (int i = 1; i < contours.size() - 2; i++) {
		...
		if (nextPos == prevPos) {
			...
			interestPoints.push_back(contours[i]);
		...
		//cv::circle(img, cv::Point(contours[i]), 3, cv::Scalar(0), 2);
...
cv::Mat maskGenerating(const cv::Mat& image) {
	...
	//printf("goodContEdge: %d , goodContBin: %d\n", goodContEdge, goodContBin);
	...
	std::vector<cv::Point> interestPoints; bool close = false;
...
int main() 
{
	try {
		...
		/*cv::Mat image = cv::imread("C:/Users/Simone/Desktop/AIA-Retinal-Vessel-Segmentation/datasets/STARE/images/im0044.ppm", CV_LOAD_IMAGE_UNCHANGED);
		std::vector<cv::Mat> channels;
