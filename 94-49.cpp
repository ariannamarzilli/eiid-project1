...
#define DRIVE_SE_SIZE_RECONSTRUCTION_EROSION 13
#define DRIVE_SE_LENGHT_SUMTOPHAT 9
#define DRIVE_SE_HEIGHT_SUMTOPHAT 17
#define DRIVE_ANGLES_SUMTOPHAT 18
#define DRIVE_VIS_RESULTS_PATH "/Users/Mariangela/Desktop/Unive rsità/Magistrale/1 anno - 2 semestre/EIID/AIA-Retinal-Vessel-Segmentation/datasets/DRIVE/results/"
...
#define CHASEDB1_SE_SIZE_RECONSTRUCTION_EROSION 20
#define CHASEDB1_SE_LENGHT_SUMTOPHAT 17
#define CHASEDB1_SE_HEIGHT_SUMTOPHAT 17
#define CHASEDB1_ANGLES_SUMTOPHAT 8
...
#define STARE_SE_SIZE_RECONSTRUCTION_EROSION 9
#define STARE_SE_LENGHT_SUMTOPHAT 9
#define STARE_SE_HEIGHT_SUMTOPHAT 9
#define STARE_ANGLES_SUMTOPHAT 12
...
int main() {
    ...
    std::string datasetPath = STARE_PATH;
    std::vector <cv::Mat> images = getImagesInFolder(datasetPath + "images",STARE_EXTENSION);
    std::vector <cv::Mat> truthsMultiChannel = getImagesInFolder(datasetPath + "groundtruths", STARE_TRUTHS_EXTENSION);
    std::vector <cv::Mat> masksMultiChannel  = getImagesInFolder(datasetPath + "masks", STARE_MASKS_EXTENSION);
    ...
    for (int i = 0; i < images.size(); i++) {
        ...
        //1°: EROSION
        cv::Mat eroded;
        cv::morphologyEx(green_channel, eroded, CV_MOP_ERODE, cv::getStructuringElement(CV_SHAPE_RECT, cv::Size(STARE_SE_SIZE_RECONSTRUCTION_EROSION, STARE_SE_SIZE_RECONSTRUCTION_EROSION)));
        ...
        tiltedSE = createTiltedStructuringElements(STARE_SE_LENGHT_SUMTOPHAT, STARE_SE_HEIGHT_SUMTOPHAT, STARE_ANGLES_SUMTOPHAT);
        cv::Mat dest;
...
/*
        ///ENHANCEMENT OF CAPILLARIES
        cv::Mat difference = foundRegion.clone();
        for (int y = 0; y < difference.rows; y++) {
            for (int x = 0; x < difference.cols; x++) {
                if (thresholded.at<unsigned char>(y,x) == 255){
                    difference.at<unsigned char>(y,x) = 0;
                }
            }
        }

        //aia::imshow("differenza", difference);

        threshold(difference, difference, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
        //aia::imshow("differenza", difference);

        thresholded = thresholded + difference;

        //aia::imshow("risultato finale", thresholded);
*/


        ///MASK EROSION
        cv::morphologyEx(masks[i], masks[i], CV_MOP_ERODE, cv::getStructuringElement(CV_SHAPE_ELLIPSE, cv::Size(STARE_SE_SIZE_MASK_EROSION, STARE_SE_SIZE_MASK_EROSION)));



        ///ESTRAZIONE FOV
        for (int y = 0; y < thresholded.rows; y++) {
            unsigned char* yRowThresh = thresholded.ptr<unsigned char>(y);
    ...
    for (int i = 0; i < visual_results.size(); i++) {
        ...
        std::string pathNameVisualResults = STARE_VIS_RESULTS_PATH + std::to_string(i+1) + ".tif";
        cv::imwrite(pathNameVisualResults, visual_results[i]);


        std::string pathNameResults = STARE_BIN_RESULTS_PATH + std::to_string(i+1) + ".tif";
