...
int main() {
    ...
    try {
        ...
        dummy = cv::imread(std::string(DATASET_PATH) + std::string("masks/20586986_6c613a14b80a8591_MG_L_ML_ANON.mask.png"), CV_LOAD_IMAGE_UNCHANGED);
        ...
        cv::threshold(roi, roi, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
            ...
            ///Equalize
            //cv::Mat resized;
            cv::Mat equalized = roi.clone();

            cv::equalizeHist(roi, equalized);
            ////cv::resize(equalized, resized, cv::Size(700, 800));
            aia::imshow("Equalized", equalized);
            ...
            clahe->setTilesGridSize(cv::Size(equalized.cols/block_size, equalized.rows/block_size));
            clahe->apply(equalized, equalized);
            //cv::resize(equalized, resized, cv::Size(700, 800));
            aia::imshow("Equalized", equalized);
            ...
            ///Mean Shift
            cv::Mat meanShift = equalized.clone();
            int hs = 10, hr = 30;
            ...
            //cv::resize(meanShift, resized, cv::Size(700, 800));
            aia::imshow("Mean Shift", meanShift);



            ///Smoothing
            /*cv::Mat smooth = meanShift.clone();
            int k = 10;
            ...
            //cv::resize(smooth, resized, cv::Size(700, 800));
            aia::imshow("Smoothed", resized);*/
            ...
            ///Closing
            cv::morphologyEx(bin, bin, CV_MOP_CLOSE, cv::getStructuringElement(CV_SHAPE_RECT, cv::Size(3, 3)));
            aia::imshow("CLOSING", bin);

            ///Opening
            cv::morphologyEx(bin, bin, CV_MOP_OPEN, cv::getStructuringElement(CV_SHAPE_RECT, cv::Size(20, 20)));
            //cv::resize(bin, resized, cv::Size(700, 800));
            aia::imshow("OPENED", bin);
            ...
            //img.setTo(cv::Scalar(255,255,255), bin);
            //cv::addWeighted(img, 0.5, img_copy_2, 0.5, 0, img_copy);
            ...
            for (int y = 0; y < bin.rows; y++) {
                for (int x = 0; x < bin.cols; x++) {
                    if (bin.at<unsigned char>(y,x) == 255) {
                        printf("bianco!\n");
